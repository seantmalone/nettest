...
def resolve_targets(cfg: Config) -> ResolvedTargets:
    ...
    for h in _expand_auto_hosts(cfg.targets.ping):
        rt.bandwidth.append(Target(kind="host", host=h))  # bandwidth uses HTTP target list in practice
    rt.bandwidth = []  # bandwidth uses cfg.targets.http; reset and recompute
    for url in cfg.targets.http:
...
class Scheduler:
    def __init__(self, bus: ResultBus):
        self._bus = bus
    ...
    def pause(self) -> None:
        if not self._paused.is_set():
            ...
            for j in self._jobs:
                j.cancel.set()
            asyncio.create_task(self._emit_marker("_paused"))
    ...
    def resume(self) -> None:
        if self._paused.is_set():
            ...
            for j in self._jobs:
                j.cancel.clear()
            asyncio.create_task(self._emit_marker("_resumed"))

    async def _emit_marker(self, kind: str) -> None:
        r = Result(
            ts=datetime.now(timezone.utc),
            host=self._jobs[0].probe.ctx.hostname if self._jobs else "?",
...
"""Bus consumer that appends Results to daily JSONL files (UTC date)."""
from __future__ import annotations
...
@respx.mock
async def test_http_probe_records_status_and_size():
...
"""HTTP probe with timing breakdown (DNS, connect, TLS, TTFB, total)."""
from __future__ import annotations
...
class HttpProbe(Probe):
    ...
    async def measure(self, target: Target) -> Result:
        ...
        async def hook_request(request: httpx.Request) -> None:
            events["request_start"] = time.perf_counter()

        async def hook_response(response: httpx.Response) -> None:
            events["response_start"] = time.perf_counter()
            await response.aread()
            events["response_end"] = time.perf_counter()
        ...
        try:
            async with httpx.AsyncClient(
                timeout=timeout,
                event_hooks={"request": [hook_request], "response": [hook_response]},
        ...
        total_ms = (events.get("response_end", time.perf_counter()) - t0) * 1000
        ttfb_ms = (events.get("response_start", t0) - events.get("request_start", t0)) * 1000
...
def parse_traceroute_output(out: str) -> list[dict]:
    ...
    for line in out.splitlines():
        ...
        if ip is None and m.group(4) == "*":
            ip = None
        times = [
            None if t == "*" else float(t.replace("ms", "").strip())
            for t in _TIME_TOKEN_RE.findall(line) for t in [t[0] if t[0] else "*"]
        ][:3]
        # The above comprehension is intentionally restrictive; fall back to a simpler scan:
...
The pattern detector subscribes to the result bus, maintains a rolling 30-second window, and emits `Event` records when configured rules fire. Events are persisted to the `events` table and tagged onto the contributing `Result` rows via the `tags` field.
...
class PatternDetector:
    ...
    def _dispatch(self, fn, r, scope) -> Event | None:
        # cooldown: don't refire same scope more than once per 5s
        key = scope
...
We split the TUI into a few small modules so each is independently testable.
...
# Nerd Font glyphs; ASCII fallbacks chosen by the renderer when unavailable
ICONS: dict[str, str] = {
    "ping":         "\U000F04C5",  # 󰓅
    "dns_cached":   "\U000F01E7",  # 󰇧
    "dns_uncached": "\U000F01E7",
    "http":         "\U000F039F",  # 󰎟  (placeholder — pick a stable glyph in implementation)
    "tcp_connect":  "\U000F04C5",
    "traceroute":   "\U000F04F9",
    "stream":       "\U000F057E",
    "mtu":          "\U000F1F86",
    "bandwidth":    "\U000F0570",
    "wifi":         "\U000F05A9",
...
def _worst(snaps: list[TargetSnapshot]) -> tuple[Severity, float]:
    if not snaps:
        return "ok", 0.0
    sev: Severity = "ok"
    worst_loss = 0.0
    for s in snaps:
        if s.loss_pct >= 5.0:
            sev = "crit"
        elif s.loss_pct >= 1.0 and sev != "crit":
            sev = "warn"
        worst_loss = max(worst_loss, s.loss_pct)
    return sev, worst_loss


def compute_health_summary(snaps: dict[tuple[str, str], TargetSnapshot]) -> list[HealthRow]:
...
- [ ] **Step 1: Write failing tests (using Textual's snapshot API)**
...
async def test_app_starts_and_renders_header():
    cfg = Config()
    bus = ResultBus()
    app = NettestApp(bus=bus, cfg=cfg, hostname="test-host")
    async with app.run_test() as pilot:
        await pilot.pause()
        # rough check: header text rendered
...
async def test_app_records_published_result():
    cfg = Config()
    bus = ResultBus()
    app = NettestApp(bus=bus, cfg=cfg, hostname="h")
    async with app.run_test() as pilot:
        await bus.publish(Result(
        ...
        await pilot.pause()
        snap = app.aggregators.get(("ping", "1.1.1.1"))
        assert snap is not None
        assert snap.snapshot().last_ms == 12.5
...
class NettestApp(App):
    CSS = """
    #health { height: 7; border: solid $accent; padding: 0 1; }
    #targets { border: solid $accent; padding: 0 1; }
    #events  { border: solid $accent; padding: 0 1; width: 50%; }
    """
    ...
    def __init__(self, bus: ResultBus, cfg: Config, hostname: str):
        super().__init__()
        self._bus = bus
        self._cfg = cfg
        self._hostname = hostname
        self.aggregators: dict[tuple[str, str], TargetAggregator] = {}
        self._events: deque[Event] = deque(maxlen=20)
    ...
    def _refresh(self) -> None:
        ...
        # health
        rows = compute_health_summary(snaps)
        text = "\n".join(
            f"  ● {row.severity:5}  {row.name:10}  {row.detail}" for row in rows
        )
        self.query_one("#health_text", Static).update(text)
        ...
        if self._events:
            ev_text = "\n".join(
                f"  {e.ts_end.strftime('%H:%M:%S')}  [{e.severity}]  {e.summary}"
                for e in list(self._events)[-8:]
            )
        else:
    ...
    def action_toggle_pause(self) -> None:
        ...
        # In integrated mode, the CLI passes a scheduler reference and toggles its pause state;
        # here we just record the intent for tests. Wiring lives in nettest.cli.
```
...
**Chunk 7 Done.** TUI is functional with live aggregator updates, health summary, target table, and event panel. Detail view, History tab, and snapshot keybinding are wired in Chunk 9.
...
async def test_results_endpoint_filters_by_probe(db: Path):
    ...
    assert resp.status_code == 200
    assert all(r["probe"] == "ping" for r in resp.json())
...
def build_app(
    ...
    async def results_route(
        ...
        **kw,
        ...
        # FastAPI doesn't allow `from` as a parameter name directly
        from_ms = int(kw.get("from", 0))
        to_ms = int(kw.get("to", int(time.time() * 1000)))
        conn = sqlite3.connect(db_path)
        try:
            res = pick_resolution(to_ms - from_ms)
            if res == "raw":
                return JSONResponse(query_results(conn, probe, target, from_ms, to_ms))
            if res == "1m":
                return JSONResponse(query_rollups_1m(conn, probe, target, from_ms, to_ms))
            return JSONResponse(query_rollups_1h(conn, probe, target, from_ms, to_ms))
        finally:
            conn.close()

    @app.get("/api/events")
    async def events_route(**kw) -> JSONResponse:
        from_ms = int(kw.get("from", 0))
        to_ms = int(kw.get("to", int(time.time() * 1000)))
        conn = sqlite3.connect(db_path)
        try:
            return JSONResponse(query_events(conn, from_ms, to_ms))
        finally:
            conn.close()

    @app.get("/api/export.csv")
    async def export_csv(
        probe: str | None = None, target: str | None = None, **kw,
    ) -> Response:
        from_ms = int(kw.get("from", 0))
        to_ms = int(kw.get("to", int(time.time() * 1000)))
        conn = sqlite3.connect(db_path)
        try:
            rows = query_results(conn, probe, target, from_ms, to_ms)
        finally:
            conn.close()
        lines = ["ts,probe,target,ok,duration_ms,error"]
        for r in rows:
            lines.append(f"{r['ts']},{r['probe']},{r['target']},{int(r['ok'])},{r['duration_ms']},{r['error'] or ''}")
    ...
    if bus is not None:
        ...
        async def ws_live(ws: WebSocket) -> None:
            await ws.accept()
            q = bus.subscribe(f"ws:{id(ws)}", drop_policy="drop_oldest", max_depth=500)
            try:
                while True:
                    r = await q.get()
...
(function () {
  ...
  function pushPoint(r) {
    ...
    series.push({ ts: new Date(r.ts), duration_ms: r.duration_ms, ok: r.ok });
...
class Runtime:
    ...
    async def run(self) -> None:
        ...
        def _persist_event(e):
            try:
                insert_event(event_db_conn, e)
            except sqlite3.Error:
                log.exception("failed to persist event")

        self.detector._on_event = _persist_event  # noqa: SLF001 (bypass setter for now)
...
def build_runtime(argv: list[str], data_dir: Path | None = None) -> Runtime:
    ...
    def _on_event(e):
        log.info("event %s: %s", e.kind, e.summary)

    detector = PatternDetector(bus=bus, cfg=cfg.patterns, on_event=_on_event)
    maintenance = StorageMaintenance(
        db_path=db_path,
...
def build_replay_app(db_path: Path) -> NettestApp:
    cfg = Config()
    # Replay mode: TUI populates from DB instead of subscribing to a live bus.
    # Using a placeholder ResultBus that never emits is enough for the existing
    # TUI scaffold; a future enhancement can stream historical rows back through it.
    from nettest.bus import ResultBus
    bus = ResultBus()
    return NettestApp(bus=bus, cfg=cfg, hostname=f"replay:{db_path.name}")
```
...
async def test_full_stack_runs_for_2s_against_local_http(tmp_path: Path):
    ...
    try:
        ...
        # add a TCP target manually
        from nettest.types import Target
        rt.scheduler.add(
            rt.scheduler._jobs[0].probe,  # tcp_connect probe (only one we asked for)
            [Target(kind="tcp", host="127.0.0.1", port=port)],
        ) if rt.scheduler._jobs else None
        await rt.run()
    finally:
        server.shutdown()
...
Expected:
- TUI renders, updates, shows real probe activity
