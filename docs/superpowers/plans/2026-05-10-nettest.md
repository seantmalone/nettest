...
class HttpProbe(Probe):
    ...
    async def measure(self, target: Target) -> Result:
        ...
        dns_ms = _phase_ms(phases, "connection.connect_tcp.started",
                           "connection.connect_tcp.started")  # DNS happens before TCP connect_started
        ...
        return Result(
            ...
            metrics={
                ...
                # All four spec-required keys are always present (None when not measurable).
                "dns_ms": dns_ms if dns_ms is not None else 0.0,
...
async def test_traceroute_probe_invokes_correct_tool(tmp_path):
    ...
    with patch("nettest.probes.traceroute.platform.system", return_value="Darwin"):
        with patch("asyncio.create_subprocess_exec") as create:
            proc = create.return_value
            proc.communicate = lambda: asyncio.sleep(0, result=(sample.encode(), b""))
            proc.returncode = 0
...
_WIN_HOP_RE = re.compile(r"^\s*(\d+)\s+(.+?)\s+([\d.]+|\*)\s*$")
...
def parse_traceroute_output(out: str) -> list[dict]:
    ...
    for line in out.splitlines():
        ...
        if not m:
            continue
        ip = m.group(3) or (m.group(2) if m.group(2) else None)
    ...
    for line in out.splitlines():
        ...
        for p in parts[1:4]:
            if p == "*":
                ...
                try:
                    rtts.append(float(p.replace("ms", "")))
                except ValueError:
                    rtts.append(None)
        ip = parts[-1]
        avg, loss = _hop_stats(rtts)
...
async def test_app_starts_and_renders_header():
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        rendered = app.screen.render().strip()
        assert "test-host" in rendered or "nettest" in rendered
    ...
    def _refresh(self) -> None:
        ...
        for (probe, target), s in sorted(snaps.items()):
            ...
            if key in self._row_keys:
                ...
                for col_idx, value in enumerate(cells):
                    table.update_cell_at((table.get_row_index(row_key), col_idx), value)
            else:
        ...
        if self._events:
            ev_text = "\n".join(
                ...
                f"{e.ts_end.strftime('%H:%M:%S')}  {e.kind}: {e.summary}"
...
```bash
git add src/nettest/tui/app.py tests/test_tui_app.py
git commit -m "feat: Textual TUI app shell with header/health/targets/events"
```
...
class EventsScreen(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        if not self._events:
            ...
            return
        lines = [
            f"{e.ts_end.strftime('%H:%M:%S')}  [{e.severity}]  {e.kind}: {e.summary}"
...
async def test_results_endpoint_uses_rollups_for_long_range(tmp_path: Path):
    ...
    for i in range(5):
        conn.execute(
            "INSERT INTO rollups_1m VALUES (?, 'ping', 'x', 60, 60, 0.0, 1.0, 2.0, 3.0, 4.0)",
            (base + i * 60_000,),
        )
    conn.commit(); conn.close()
    app = build_app(db_path=p, hostname="h")
